...
export interface CoverflowCarouselProps {
  ...
  loop?: boolean;
  showCaption?: boolean;
  /** Below the stage, or overlaid on each card. */
  captionPlacement?: "below" | "overlay";
  showPagination?: boolean;
  showNavigation?: boolean;
  /** Advance one card at a time. Pass a delay in ms, or true for 4000. */
  autoplay?: boolean | number;
  /** Pause autoplay while the pointer is over a card. */
...
export function CoverflowCarousel({
  ...
  const overlay = showCaption && captionPlacement === "overlay";
  const autoplayDelay =
    autoplay === true ? 4000 : typeof autoplay === "number" ? autoplay : 0;
  ...
  }, [autoplayDelay, count, nudge, paused]);
  ...
  return (
    ...
    >
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                onMouseEnter={
                  autoplayDelay && pauseOnHover ? pauseAutoplay : undefined
                }
                onMouseLeave={
              ...
              >
                ...
                {overlay && (slide.title || slide.subtitle) ? (
                  <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-linear-to-t from-black/80 via-black/50 to-transparent px-4 pt-16 pb-4 text-ivory">
                    {slide.title ? (
                      <p className="font-heading text-[15px] font-medium tracking-wide">
                        {slide.title}
                      </p>
                    ) : null}
                    {slide.subtitle ? (
                      <p className="mt-1 line-clamp-3 text-[11px] leading-snug italic text-ivory/90">
                        {slide.subtitle}
                      </p>
                    ) : null}
                    {slide.meta && slide.meta.length > 0 ? (
                      <p className="mt-2 text-[10px] tracking-[0.14em] text-ivory/70 uppercase">
      ...
      {showCaption && captionPlacement === "below" && active?.title && (
        <div
          key={selected}
          className={cn(
            "mt-2 flex flex-col items-center px-6 duration-300 animate-in fade-in",
            captionClassName,
          )}
        >
          {active.href ? (
            <a
              href={active.href}
              className="text-[15px] font-semibold tracking-tight text-foreground outline-none hover:text-gold focus-visible:ring-2 focus-visible:ring-ring"
            >
              {active.title}
            </a>
          ) : (
