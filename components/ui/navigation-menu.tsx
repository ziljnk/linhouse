...
function NavigationMenu({
  ...
  return (
    ...
    >
      {children}
      <NavigationMenuPositioner align={fullWidth ? "start" : align} fullWidth={fullWidth} />
    </NavigationMenuPrimitive.Root>
  )
}
...
function NavigationMenuPositioner({
  ...
  fullWidth = false,
  ...props
}: NavigationMenuPrimitive.Positioner.Props & { fullWidth?: boolean }) {
  return (
    <NavigationMenuPrimitive.Portal>
      <NavigationMenuPrimitive.Positioner
        side={side}
        sideOffset={fullWidth ? 0 : sideOffset}
        align={align}
        alignOffset={alignOffset}
        className={cn(
          "isolate z-50 h-(--positioner-height) w-(--positioner-width) max-w-(--available-width) transition-[top,left,right,bottom] duration-[0.35s] ease-[cubic-bezier(0.22,1,0.36,1)] data-instant:transition-none data-[side=bottom]:before:top-[-10px] data-[side=bottom]:before:right-0 data-[side=bottom]:before:left-0",
          fullWidth &&
            "left-0! w-screen! max-w-none! translate-none! h-auto!",
          className
        )}
        {...props}
      >
        <NavigationMenuPrimitive.Popup
          className={cn(
            ...
            fullWidth &&
              "xs:w-screen h-auto min-h-0 w-screen max-w-none rounded-none border-b border-border shadow-[0_18px_40px_rgba(0,0,0,0.08)] ring-0 data-ending-style:scale-100 data-starting-style:scale-100"
          )}
        >
          <NavigationMenuPrimitive.Viewport
            className={cn(
              "relative size-full overflow-hidden",
              fullWidth && "h-auto w-full overflow-visible"
            )}
          />
        </NavigationMenuPrimitive.Popup>
      </NavigationMenuPrimitive.Positioner>
